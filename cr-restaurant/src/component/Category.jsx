  ...
  <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-7 lg:grid-cols-9 gap-4 p-5">
    {menu_list.map((item, index) => {
      return (
        <Link
          to={`/categoryItem/${item.menu_name}`}
          key={index}
          className="flex flex-col items-center gap-2 overflow-hidden sticky-wrapper"
        >
          <div className="img-wrapper border-[3px] border-custom">
            <img
              className="h-24 w-24 rounded-[50%] category-item"
              src={item.menu_image}
              alt=""
            />
          </div>
          <p className="text-sm font-cart-font text-gray-600 font-semibold category-item">
            {item.menu_name}
          </p>
        </Link>
      );
